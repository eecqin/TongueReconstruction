...
blka = zeros(2*P,2*P);
ba = zeros(2*P,1);
iA00 = [0.35 -2.5; 0.5 0.35]; ib00 = [150; -50]; % original [5;-5]

iA0 = [0.5 -0.1; 0.3 0.3]; ib0 = [60; 10]; % original [5;-5]
iA1 = [1.1*iA0]; ib1 = [ib0+[0;0]];
iA2 = [1.0*iA0]; ib2 = [ib0+[2;3]];
iA3 = [1.2*iA0]; ib3 = [ib0+[-5;-5]];
for p=1:8
...
yTE3 = zeros(size(yTE));
G = randn(2*P,2*P,2*P)/(3*1e+5);
figure(123);clf;
for i=1:size(yTE,1)
    z = yTE(i,:)';
    for p=1:2*P
        yTE3(i,p) = 0.5*z'*G(:,:,p)*z + blka(p,:)*z + 0*ba(p);
    end
end
...
for i=1:size(yTE,1)
  ...
  subplot(1,2,1)
  plot(yTE(i,1:2:end-1),yTE(i,2:2:end),'bo-');hold on;
  plot(yTE3(i,1:2:end-1),yTE3(i,2:2:end),'ro-');
  set(gca,'DataAspectRatio',[1 1 1]); axis ij; 
  axis([-90 160 0 100]);
  legend('old speaker','new speaker');
  title('Nonlinear')
  subplot(1,2,2)
  plot(yTE(i,1:2:end-1),yTE(i,2:2:end),'bo-');hold on;
  plot(yTE2(i,1:2:end-1),yTE2(i,2:2:end),'ro-');
